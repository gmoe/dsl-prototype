...
object Structures {
  ...
  case class Note(val primitive: Primitive, val duration: Beat) extends Music
  ...
  implicit class ChordBuilder(val p: Pitch) extends AnyVal {
    ...
    def P5(): List[Pitch] = p :: Interval(5, IntervalQuality.Perfect).fromPitch(p) :: Nil

    def majTriad: List[Pitch] = p :: Interval(3, IntervalQuality.Major).fromPitch(p) ::
      Interval(5, IntervalQuality.Perfect).fromPitch(p) :: Nil

    def minTriad: List[Pitch] = p :: Interval(3, IntervalQuality.Minor).fromPitch(p) ::
      Interval(5, IntervalQuality.Perfect).fromPitch(p) :: Nil
